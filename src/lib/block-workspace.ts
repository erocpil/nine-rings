...
export function openBlockWorkspace(editor: Editor, position: number | undefined, trigger: HTMLElement, startInEditMode = false, selectedPositions?: number[]) {
  ...
  editor.view.dom.dispatchEvent(new CustomEvent(OPEN_BLOCK_WORKSPACE, {
    bubbles: true, detail: { position, trigger, restoreFocus, startInEditMode, selectedPositions },
  }));
}
