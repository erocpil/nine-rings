...
| **EPUB 阅读** | 本地导入无 DRM 的 EPUB 2/3，支持目录、章节导航、阅读进度、字号及浅色/护眼/深色主题 |
...
手动 JSON 导出、紧急恢复文件和 GitHub 全量快照共用相同备份格式，包含笔记、待办、正文书签、应用配置、界面相关的非敏感用户设置，以及最后打开的文档与其光标/滚动位置。恢复成功后应用会自动重新载入并立即应用这些设置。本地 PDF、EPUB 原文件及其设备阅读进度不进入备份；GitHub Token、密码、密钥和授权凭据始终排除，恢复后需要在当前设备重新提供。
