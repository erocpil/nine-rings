...
手动 JSON 导出、紧急恢复文件和 GitHub 全量快照共用相同备份格式，包含笔记、待办、正文书签、应用配置、界面相关的非敏感用户设置，以及最后打开的文档与其光标/滚动位置。恢复成功后应用会自动重新载入并立即应用这些设置。GitHub Token、密码、密钥和授权凭据始终排除，恢复后需要在当前设备重新提供。
