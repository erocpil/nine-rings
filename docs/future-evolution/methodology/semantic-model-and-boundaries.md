...
实践记录应保留反例、未解决冲突、被忽略的 Feedback 及忽略原因，并在 Review 时主动提示可能的确认偏误。仅靠提醒不足以消除动机性推理、自利性归因或选择性取样；对高风险、不可逆、涉及他人利益或重复重大失败的事项，应引入与原 Decision 分离的反证步骤或独立质询。质询者应能查看原始材料、提出替代解释，并将异议与最终结论一并保留；其角色不是为原结论背书。

任何 World Model 都只是当前有效 Claim、证据、冲突和不确定性的可追溯投影，而不是对复杂系统的完整因果图。系统应明确区分观察性、预测性和因果性 Claim：单次 Outcome 或相关性通常只能改变前两者的强度，不能单独产生强因果 Revision。遇到意外信号、样本不足、来源不独立或环境结构变化时，应降低结论状态并重新打开 Unknown，而非增加模型细节来维持确定感。
