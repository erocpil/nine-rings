...
| 项目 | 说明 | 状态 |
|------|------|------|
| **编辑模型契约** | 明确 ProseMirror、Quill Delta 与 Markdown 的权威边界，补充格式属性映射和往返测试 | ⬜ 待做 |
| **脏块增量序列化** | 按发生变化的块转换和保存，避免大文档每次保存都执行全量 Delta 转换 | ⬜ 待做 |
| **编辑模式局部挂载** | 对远离视口的代码块、表格、图片和 Mermaid 视图延迟创建，降低只读切编辑时的卡顿 | ⬜ 待做 |
| **只读与编辑渲染统一** | 复用块级渲染组件、样式变量和折叠状态，减少虚拟只读视图与完整编辑器的行为差异 | ⬜ 待做 |
| **块工作区同步协议** | 为局部编辑器记录源文档版本，明确外部更新时的合并、重载和冲突提示策略 | ⬜ 待做 |
