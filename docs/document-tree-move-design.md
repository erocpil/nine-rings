...
> 实现状态（2026-07-30）：已接入统一 `StorageAdapter` 的单篇文档移动和目录原子移动，
> Web/IndexedDB 与 Tauri/SQLite 均支持；文档树右键菜单已提供基础“移动到…”入口，当前使用
> 完整目标路径输入。可视化目录选择器、拖拽移动和跨端 E2E 仍待继续完善。
...
- [x] `daily` 不能被移动、重命名或作为目标。
- [x] 目录不能移动到自身或自身后代。
- [x] `projects/a` 的移动不会影响 `projects/abc`。
- [ ] 合并目标需要明确确认。
- [x] 软删除文档不随活动目录移动。
- [x] 移动不改变正文、标题、更新时间或版本历史。
- [x] 中途失败不会留下部分移动。
- [x] 当前打开文档移动后保持选中。
- [ ] 树计数、MOC 和折叠状态正确刷新。
