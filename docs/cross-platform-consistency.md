...
- 桌面 Web / Tauri 使用左侧竖排入口栏，在 `app-sidebar` 中切换文档树、文档列表、PDF/EPUB 阅读器。重复点击当前入口收起侧栏，拖动分隔条调整宽度；阅读器与右侧编辑器同时存在，切换侧栏面板保留阅读器，正文编辑不触发阅读器快捷键。摘录新建文档后继续保留侧栏原书。手机保留左上/左下右划的两种抽屉和整页阅读器，不采用桌面分栏。
