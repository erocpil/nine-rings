...
describe("shared edge swipe rules", () => {
  ...
  it.each([1, -1])(
    "uses identical distance and direction rules on side %s",
    (sign) => {
...
describe("edge swipe lifecycle", () => {
  ...
  function setup() {
    ...
    const run = vi.fn();
    const cleanup = bindEdgeSwipe(host as unknown as Window, () => ({
      direction: "right",
      run,
    }));
  ...
  it.each([
    ...
    "identifier",
    "native-scroll",
  ])("does not complete a gesture interrupted by %s", (reason) => {
    ...
    if (reason === "multitouch")
      send("touchstart", [touch(8), touch(10, 0, 2)]);
    ...
    if (reason === "native-scroll")
      send("touchmove", [touch(10)], [touch(10)], false);
