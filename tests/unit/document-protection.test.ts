...
import { createDocumentKey, decryptDocument, encryptDocument, isEncrypted, openDocumentSession, unlockDocument } from "../../src/lib/document-crypto";
...
const body = { ops: [{ insert: "不可搜索的正文 secret-body\n" }], metadata: { bookmarks: [{ id: "bookmark", position: 1, preview: "不可泄露的书签", createdAt: "now" }] } };
beforeEach(async () => {
  vi.stubGlobal("crypto", webcrypto);
  await withDB(async db => {
    const stores = ["notes", "note_versions", "images", "protected_paths", "daily_pages"];
...
describe("document encryption", () => {
  ...
  it("authenticated encryption preserves Unicode/metadata, randomizes every save and rejects wrong passwords/tampering", async () => {
    ...
    await expect(unlockDocument(a, password.trim() + "wrong")).rejects.toThrow();
    const corrupt = structuredClone(a); corrupt.encrypted!.data = "AAAA" + corrupt.encrypted!.data.slice(4);
  ...
  it("encrypts current and historic content; search never sees plaintext even while unlocked; edits and backups remain ciphertext", async () => {
    ...
    const index = new NoteSearchIndex(); index.rebuild([stored]);
    ...
    await adapter.updateNote(note.id, { content: { ops: [{ insert: "新的秘密 edited-secret\n" }] } });
    close();
    stored = (await adapter.getNote(note.id))!;
    expect((await unlockDocument(stored.content, password)).content.ops[0].insert).toContain("edited-secret");
    ...
    await expect(adapter.exportNoteMarkdown(note.id)).rejects.toThrow("加密文档");
    await setDocumentPassword(note.id, true);
    expect(isEncrypted((await adapter.getNote(note.id))!.content)).toBe(false);
    expect(isEncrypted((await adapter.getNoteVersions(note.id))[0].content)).toBe(false);
  });
  it("path password covers descendants and future imports; empty paths survive deletion, export/import and safe merge", async () => {
    ...
    await setPathPassword("areas/private/");
    for (const id of [a.id, b.id]) expect((await unlockDocument((await adapter.getNote(id))!.content, password)).content).toEqual(body);
    const c = await create("新文档", "areas/private/new");
    expect(isEncrypted(c.content)).toBe(true);
    await adapter.upsertNote({ date: c.date, title: c.title!, storagePath: c.storagePath, content: body });
    ...
    expect(tree.find(n => n.path === "areas/private")).toMatchObject({ protectionRoot: true, count: 0 });
    ...
    const merged = buildSafeMergedBackup(empty, JSON.stringify({ version: 1, notes: [], daily_pages: [] })).json;
    ...
    expect((await adapter.getPathTree(false)).some(n => n.path === "areas/private")).toBe(false);
  });
  it("moves documents across protection boundaries without implicit plaintext and preserves path identity on rename", async () => {
    ...
    expect((await listProtectedPaths())[0]).toMatchObject({ id: pathId, path: "areas/renamed" });
    await expect(setPathPassword("areas/renamed/child")).rejects.toThrow("父子路径");
  });
  it("CAS rejects concurrent changes without rewriting any note or path", async () => {
    const note = await create();
    const before = await readProtectionState();
    const after = structuredClone(before); after.notes[0].title = "不应写入";
    await adapter.updateNote(note.id, { title: "其他窗口" });
    await expect(commitProtectionState(before, after)).rejects.toThrow("其他窗口");
  ...
  it("raw search_text and bookmark metadata disappear atomically with encryption", async () => {
    const note = await create();
    await setDocumentPassword(note.id);
    const records = await withDB(db => getAll(db.transaction("notes").objectStore("notes")));
