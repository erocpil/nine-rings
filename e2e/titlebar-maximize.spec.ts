...
async function mountTitlebar(
  ...
  await page.evaluate(
    async ({ platform, failOnce, hangOnce, exhibition }) => {
      ...
      )) as typeof import("@tauri-apps/api/mocks");
      const { invoke } = (await load(
        "/node_modules/@tauri-apps/api/core.js",
      )) as typeof import("@tauri-apps/api/core");
      ...
      mockIPC(
        (command, args) => {
          ...
          if (
            command === "toggle_window_maximize" ||
            command.endsWith("|internal_toggle_maximize")
          ) {
            ...
            if (rejectNext) {
              rejectNext = false;
              throw new Error("test maximize failure");
            }
            maximized = !maximized;
        ...
        React.createElement(
          ...
          exhibition
            ? React.createElement(
                "select",
                { "aria-label": "测试风格" },
                React.createElement("option", null, "清雅"),
              )
            : undefined,
        ),
      );
    },
    { platform, failOnce, hangOnce, exhibition },
  );
...
async function commands(page: Page) {
  return page.evaluate(
    () => JSON.parse(document.body.dataset.windowCommands || "[]") as string[],
  );
}

test("macOS 双击标题文字和空白只最大化/还原，不进入全屏或重复切换", async ({
  page,
}) => {
  await mountTitlebar(page);
  await page.locator(".titlebar-title").dblclick();
  await expect
    .poll(() => page.evaluate(() => document.body.dataset.maximized))
    .toBe("true");
  ...
  expect(
    log.filter((command) => command === "toggle_window_maximize"),
  ).toHaveLength(2);
...
test("macOS 双击容许轻微移动，拖动、右键及窗口按钮不触发最大化", async ({
  page,
}) => {
  await mountTitlebar(page);
  const bar = page.locator(".titlebar");
  const gesture = async (distance: number, button = 0) => {
    await bar.dispatchEvent("mousedown", {
      detail: 2,
      button,
      clientX: 400,
      clientY: 18,
    });
    await bar.dispatchEvent("mouseup", {
      detail: 2,
      button,
      clientX: 400 + distance,
      clientY: 18,
    });
  };
  await gesture(2);
  ...
  await gesture(0, 2);
  await bar
    .locator(".titlebar-controls")
    .dispatchEvent("dblclick", { detail: 2 });
  ...
  expect(
    (await commands(page)).filter(
      (command) => command === "set_window_fullscreen",
    ),
  ).toHaveLength(1);
});
...
test("Windows 双击最大化后的全屏按钮通过原生统一入口进入和退出", async ({
  page,
}) => {
  await mountTitlebar(page, "Win32");
  await page.locator(".titlebar-title").dblclick();
  ...
  await expect(
    page.getByRole("button", { name: "进入全屏", exact: true }),
  ).toBeVisible();
  const log = await commands(page);
  expect(
    log.filter((command) => command === "set_window_fullscreen"),
  ).toHaveLength(2);
...
test("展陈标题栏保留窗口操作，工作区和配色控件不触发最大化", async ({
  page,
}) => {
  ...
  await page.getByRole("button", { name: "测试工作区" }).dblclick();
  await page
    .getByLabel("测试风格")
  ...
  expect(await commands(page)).toContain("hide");
});
