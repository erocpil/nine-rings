...
test.skip(
  process.env.NR_EDITOR_BENCHMARK !== "1",
...
for (const count of [1500]) {
  test(`${count} 块已有文档：完整可编辑正文首次挂载三次`, async ({
    ...
    const id = `opening-production-${count}`;
    await page.evaluate(
      async ({ id, count }) => {
    ...
    for (let repetition = 0; repetition < 3; repetition++) {
      // Fresh page/editor session each time; HTTP caches may be warm.
        ...
        await expect
          .poll(
            () =>
              sample.evaluate(
                () =>
                  (window as unknown as { __openingProbe?: unknown })
                    .__openingProbe,
              ),
            { timeout: 60000 },
          )
        ...
        console.log(
          ...
          JSON.stringify({
            ...
            ...(await sample.evaluate(
              () =>
                (window as unknown as { __openingProbe: object })
                  .__openingProbe,
            )),
          }),
        );
        await expect(sample.locator(".ProseMirror")).toBeVisible();
        await expect(sample.locator(".ProseMirror")).toBeEditable();
        await expect(sample.locator(".ProseMirror > *")).toHaveCount(count);
        await sample.close();
      }
    }
  });
