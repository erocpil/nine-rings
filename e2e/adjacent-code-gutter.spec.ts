...
for (const width of [1280, 390])
  test.describe(`${width}px`, () => {
    ...
    for (const delayed of [false, true])
      test(`紧邻的折叠代码块及后续块号保持对齐${delayed ? "（延迟观察回调）" : ""}`, async ({
        page,
      }) => {
          ...
          const note = await api.notes.create({
            ...
            content: {
              ...
              content: [
                ...
                list,
                { ...code, attrs: { ...code.attrs, title: "中序遍历版本" } },
                { ...code, attrs: { ...code.attrs, title: "上下界递归版本" } },
                {
                  type: "heading",
                  attrs: { level: 2 },
        ...
        const check = async () => {
          for (const index of [406, 407, 408, 409, 410, 411, 412]) {
        ...
        await align();
        await check();
        // Typing changes later block positions without changing the block count.
        await editor.evaluate((element) => {
          const instance = (element as HTMLElement & { editor: Editor }).editor;
          let codePosition = 0;
          instance.state.doc.forEach((node, pos, index) => {
            if (index === 406) codePosition = pos;
          });
          instance.view.dispatch(
            instance.state.tr.insertText(
        ...
        const revisit = async () => {
          ...
          await editor.evaluate((element) => {
            const scroll = element.closest(".note-editor-scroll")!;
            scroll.scrollTop = 0;
            scroll.dispatchEvent(new Event("scroll"));
          });
          await page.waitForTimeout(200);
        ...
        await expect(code.locator("pre code")).not.toContainText(
          "additional implementation",
        );
        await revisit();
        await code
